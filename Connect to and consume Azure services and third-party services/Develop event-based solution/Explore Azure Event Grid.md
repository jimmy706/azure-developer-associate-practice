...
![Azure Event Grid](https://learn.microsoft.com/en-us/training/wwl-azure/azure-event-grid/media/functional-model.png)
...
### Topics
The Event Grid topic provides an endpoint where the source sends events. The publisher creates the Event Grid topic, and decides whether an event source needs one topic or more than one topic. A topic is used for a collection of related events. To respond to certain types of events, subscribers decide which topics to subscribe to. There are two types of topic:
...
## Output batching
For improve HTTP performance in high-throughput scenarios, you can configure Event Grid to delivery events in batch. Batched delivery has two settings:
...
## Delayed delivery
To protect unhealthy endpoints from overwhelm by retried requests, delayed delivery is used. 
